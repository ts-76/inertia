...
import { createInertiaApp, Head, usePage } from '../packages/hono-jsx/src/index'
import { createElement } from '../packages/hono-jsx/node_modules/hono/dist/jsx/index.js'
import { renderToString } from '../packages/hono-jsx/node_modules/hono/dist/jsx/dom/server.js'
...
test('boots from script JSON and renders the initial page', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Hono JSX Home' })).toBeVisible()
...
test('supports Head, layout, remember, form state, deferred, visible, poll, and prefetch helpers', async ({ page }) => {
  ...
  await expect(page).toHaveTitle('Hono JSX Test Home alpha')
  ...
  await expect(page).toHaveTitle('Hono JSX Test Home beta')
